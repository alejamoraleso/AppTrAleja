//   Creacion de las variables de los elementos a utilizar
...
divListas.addEventListener("mouseenter", ()=>{
  let items = document.getElementsByClassName("col"); //llamado de todos los elementos con la class col 
  
    for (let i = 0; i < items.length; i++) {    //------con el for se iteran las listas que contienen class col
      new Sortable(items[i], {                  //------se crea new sortable para arrastrar y soltar tareas
...
// Crear new sortable para mover las listas (columnas)
...
// Con esta funcion se muestra el formulario de crear tarea
...
// Esta funcion se usa para mostrar formulario de crear listas
...
// Esta funcion se usa para ocultar btns de menu al abrir hoja de calculo como bd
btnBd.addEventListener("click",()=>{   
  checkbox.checked = false;
});

// Esta funcion se usa para cerrar formulario de crear tarea
btnCerrarFCT.addEventListener("click",()=>{
  formTarea.reset(); 
  formTarea.style.left = "-80%"; 
});

//Esta funcion se usa para cerrar formulario de crear lista
btnCerrarFCC.addEventListener("click",()=>{
  formTituloColumna.reset();
  formTituloColumna.style.left = "-70%";
});

// Esta funcion se usa para crear tarea
btnCrearTarea.addEventListener("click",()=>{
  //Se validan que los campos de texto tengan informacion
  if(formTarea[0].value == "" || formTarea[1].value == "" || formTarea[2].value == "" || formTarea[3].value == ""){
    formTarea[0].focus();                //-------Cuando no haya datos colocamos el puntero en la primer caja de texto
    info.style.left = "40%"              //-------Se muestra mensaje de informacion  
    info.innerHTML = "Campos son obligatorios" //----- Se agrega el mensaje a mostrar
    setTimeout("info.style.left = '-60%'",2000);//---- Damos dos segundos de tiempo en vista al mensaje
  }else{
    postDatosTareaBd(formTarea);         //------- Se llama la funcion que manda los datos a la bd y pasamos por parametro objeto form  
  }
});

// Esta funcion se usa para para crear tarea 
...
//Esta funcion se usa para eliminar tarea 
...
// Esta funcion se usa para movernos el focus con tecla enter en form tarea
